...
| GitHub stats panel | **Cut, then reversed 2026-08-29** | Requested in the 2026-08-27 review call. Fails "something people need" — the users are a gym crew and a CFA study pair — and costs demo time it cannot repay. Bio, avatar and socials ship; GitHub survives as one linkable social. **Reversed 2026-08-29:** the product owner asked for this exact 21st.dev calendar component by name (Task 21) and supplied its source. Ships gated on a non-empty GitHub handle, adapted to the palette and voice — see `components/ui/retro-space-shooter-git-hub-calendar.tsx`. |
